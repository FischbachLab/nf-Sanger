...
bash /work/scripts/16S_assembly_silva.sh ${group} ${LOCAL_OUTPUT}/QB_RAW_DATA_by_group/${group} \
> ${LOG_DIR}/$group.log 2>&1 \
&& bash /work/scripts/16S_assembly_ncbi.sh ${group} ${LOCAL_OUTPUT}/QB_RAW_DATA_by_group/${group}

cd ${LOCAL_OUTPUT}/QB_RAW_DATA_by_group/${group}/789F_907R_28F_1492R_outputs/

Rscript /work/scripts/sanger_assembly_summary.R
